...
const Clutter=imports["gi"]["Clutter"];const Backgrounds=imports["backgrounds"];const Utils=imports["utils"];let Parser=objectThatDelegatesTo(OMeta,{
...
"bgValue":function(){var $elf=this,_fromIdx=this.input.idx,n;n=this._many1(function(){this._not(function(){return this._applyWithArgs("exactly","]");});return this._apply("anything");});return n.join("");},
"background":function(){var $elf=this,_fromIdx=this.input.idx,n;return this._or(function(){return (function(){switch(this._apply('anything')){case "camera":return ["background",new Backgrounds.Camera()];default: throw fail}}).call(this);},function(){n=this._apply("bgValue");this._pred(Utils.isMimeType(n,"image/"));return ["background",new Backgrounds.Image(n)];},function(){n=this._apply("bgValue");this._pred(Utils.isMimeType(n,"video/"));return ["background",new Backgrounds.Video(n)];},function(){n=this._apply("bgValue");this._pred(Utils.isValidColor(n));return ["background",new Backgrounds.Color(Utils.colorFromString(n))];});},
"styleName":function(){var $elf=this,_fromIdx=this.input.idx;this._apply("spaces");return this._consumedBy(function(){this._apply("letter");return this._many1(function(){return this._or(function(){return this._apply("letter");},function(){return this._apply("digit");},function(){return (function(){switch(this._apply('anything')){case "-":return "-";default: throw fail}}).call(this);});});});},
"styleValue":function(){var $elf=this,_fromIdx=this.input.idx,k,v;return this._or(function(){k=this._apply("styleName");this._applyWithArgs("token","=");v=this._many1(function(){this._not(function(){return this._applyWithArgs("exactly","]");});return this._apply("anything");});return [k,v.join("")];},function(){k=this._apply("styleName");return [k,k];});},
"style":function(){var $elf=this,_fromIdx=this.input.idx,v;v=this._apply("styleValue");this._pred(Parser.isKeyword(v[(0)]));return Parser.getValue(v[(0)],v[(1)]);},
...
"headerProperty":function(){var $elf=this,_fromIdx=this.input.idx,d,p;d=this._apply("anything");this._apply("spacesNoNl");p=this._apply("property");return this._applyWithArgs("appendProperty",d,p);},
"slideProperty":function(){var $elf=this,_fromIdx=this.input.idx,s,p;s=this._apply("anything");this._apply("spacesNoNl");p=this._apply("property");return this._applyWithArgs("appendProperty",s,p);},
...
"slide":function(){var $elf=this,_fromIdx=this.input.idx,s;s=this._apply("newSlide");this._applyWithArgs("exactly","-");this._applyWithArgs("exactly","-");this._apply("spacesNoNl");this._many(function(){return this._applyWithArgs("slideProperty",s);});this._apply("spacesNoNl");this._applyWithArgs("exactly","\n");this._applyWithArgs("slideContent",s);return s;},
"shell":function(){var $elf=this,_fromIdx=this.input.idx;return this._or(function(){return (function(){switch(this._apply('anything')){case "#":this._applyWithArgs("exactly","!");this._many(function(){this._not(function(){return this._apply("space");});return this._apply("anything");});this._many(function(){this._not(function(){return this._applyWithArgs("exactly","\n");});return this._apply("anything");});return this._applyWithArgs("exactly","\n");default: throw fail}}).call(this);},function(){return this._apply("empty");});},
"document":function(){var $elf=this,_fromIdx=this.input.idx,d,sds;d=this._apply("newDocument");this._apply("shell");this._many(function(){return this._or(function(){return this._applyWithArgs("headerProperty",d);},function(){return this._many1(function(){return this._apply("space");});});});sds=this._many1(function(){return this._apply("slide");});this._apply("end");return ({"properties": d["properties"],"slides": sds});}});let returnValue=(function (value){return (function (){return value;});});let identity=(function (value){return value;});let colorBackground=(function (value){return new Backgrounds.Color(Utils["colorFromString"]);});(Parser["_keywords"]=({"fill": ({"name": "background-gravity","getValue": returnValue(Clutter["ContentGravity"]["RESIZE_FILL"])}),"fit": ({"name": "background-gravity","getValue": returnValue(Clutter["ContentGravity"]["RESIZE_ASPECT"])}),"font": ({"name": "font","getValue": identity}),"markup": ({"name": "use-markup","getValue": returnValue(true)}),"no-markup": ({"name": "use-markup","getValue": returnValue(false)}),"duration": ({"name": "duration","getValue": parseFloat}),"text-color": ({"name": "text-color","getValue": Utils["colorFromString"]}),"text-align": ({"name": "text-align","getValue": Utils["getTextAlignment"]}),"shading-opacity": ({"name": "shading-opacity","getValue": parseFloat}),"transition": ({"name": "transition","getValue": identity}),"command": ({"name": "command","getValue": identity}),"gravity": ({"name": "gravity","getValue": Utils["getGravity"]})}));(Parser["isKeyword"]=(function (key){return (this["_keywords"].hasOwnProperty(key) || Utils.isGravity(key));}));(Parser["getValueName"]=(function (key){if(this["_keywords"].hasOwnProperty(key)){return this["_keywords"][key]["name"];}else{if(Utils.isGravity(key)){return "gravity";}else{undefined;};}return key;}));(Parser["getValue"]=(function (key,value){let nkey=this.getValueName(key);let nvalue=value;if(this["_keywords"].hasOwnProperty(key)){(nvalue=this["_keywords"][key].getValue(value));}else{if(this["_keywords"].hasOwnProperty(nkey)){(nvalue=this["_keywords"][nkey].getValue(value));}else{undefined;};}return [nkey,nvalue];}));(Parser["newDocument"]=(function (){return ({"properties": ({}),"slides": []});}));(Parser["newSlide"]=(function (){return ({"comments": [],"content": [],"properties": ({})});}));(Parser["appendComment"]=(function (slide,comment){slide["comments"].push(comment);}));(Parser["appendContent"]=(function (slide,content){slide["content"].push(content);}));(Parser["appendProperty"]=(function (container,tuple){(container["properties"][tuple[(0)]]=tuple[(1)]);}));let parse=(function (str){let indexToPosition=(function (source,idx){let linePos=(0),lineNum=(0);for(let i=(0);(i < idx);i++){if((source.charAt(i) == "\n")){(linePos=i);lineNum++;}else{undefined;};}return ({"line": (lineNum + (1)),"offset": (idx - linePos)});});let document=undefined;try {(document=Parser.matchAll(str,"document"));}catch(e){let pos=indexToPosition(str,e["idx"]);log(((("Parsing error at : line " + pos["line"]) + " offset ") + pos["offset"]));throw e;}finally{undefined;}return document;})
